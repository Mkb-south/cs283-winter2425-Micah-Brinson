...
int get_student(int fd, int id, student_t *s)
{
    ...
    ssize_t read_bytes = read(fd, s, sizeof(student_t));

    if (read_bytes != sizeof(student_t)) {
        return ERR_DB_FILE; 
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    ssize_t written_bytes = write(fd, new_student, sizeof(student_t));
    free(new_student);

    if (written_bytes == -1) {
        printf("%s/n",M_ERR_DB_WRITE);
        close(fd);
        return ERR_DB_FILE;
    } else {
...
student_t temp_student;
    ...
    ssize_t written_bytes = write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t));
...
int count_db_records(int fd)
{
    ...
    int count = 0;
    student_t db_record;
    memset(&db_record, 0, sizeof(student_t));

    student_t temp;
    while (read(fd, &temp,sizeof(student_t)) == sizeof(student_t)) {
        if (memcmp(&temp, &db_record, sizeof(student_t)) != 0) {
            count++;
        }
    }
...
int print_db(int fd)
{
   ...
   if (count == 0) {
    ...
    student_t *temp_record = (student_t*)malloc(sizeof(student_t));
    bool first = true;
    while (read(fd, temp_record,sizeof(student_t)) == sizeof(student_t)) {
        if (memcmp(temp_record, &db_record, sizeof(student_t)) != 0) { 
            if (first) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");
                first = false;
            }
            
            float calculated_gpa_from_temp = temp_record->gpa / 100.0; 
            printf(STUDENT_PRINT_FMT_STRING, temp_record->id, temp_record->fname, temp_record->lname, calculated_gpa_from_temp);
        }
    }
    return NO_ERROR;
}
